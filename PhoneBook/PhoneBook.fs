...
module PhoneBook =
    ...
    let writeContactsToFile (contacts: Contact list) phoneBaseFile =
        if contacts = [] then ()
            ...
            let json = JsonConvert.SerializeObject((contacts @ (readContactsFromFile phoneBaseFile)), Formatting.Indented)
            File.WriteAllText(phoneBaseFile, json)

    let isContactInFile nameForFind phoneForFind phoneBaseFile =
        List.exists(fun contact -> contact.Name = nameForFind || contact.Phone = phoneForFind) <| readContactsFromFile phoneBaseFile
    ...
    let writePhoneBaseToFile (phoneBase: Contact List) phoneBaseFile =
        writeContactsToFile (List.filter(fun contact -> not (isContactInFile contact.Name contact.Phone phoneBaseFile)) phoneBase) phoneBaseFile
    ...
    let addContactToBase name phone (phoneBase: Contact List)  =
        if (not (isContactInBase name phone phoneBase)) && isCorrectPhone(phone)
            then (({Name = name; Phone = phone} :: phoneBase), true)
        else (phoneBase, false)
