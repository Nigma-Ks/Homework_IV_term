...
module LambdaInterpreter =
    ...
    let rec substitution term x substitutionTerm =
        match term with
            | Variable (y) -> if x = y then substitutionTerm
                              else term
            | Application (fstTerm, sndTerm) ->
                Application (substitution fstTerm x substitutionTerm, substitution sndTerm x substitutionTerm)
            | LambdaAbstraction (y, internalTerm) ->
                if x = y || not (IsFV internalTerm x) then term
                else
                    if IsFV substitutionTerm y then
                        let newY = (y + "'")
                        let newInternalTerm = alphaConversion internalTerm y newY
                        LambdaAbstraction (newY, substitution newInternalTerm x substitutionTerm)
                    else LambdaAbstraction (y, substitution internalTerm x substitutionTerm) 
        
    let rec internalBetaReduction term =
        match term with
            | Variable _ -> term
            | LambdaAbstraction (var, internalTerm) ->
                LambdaAbstraction (var, internalBetaReduction internalTerm)
            | Application (fstTerm, sndTerm) ->
                match fstTerm with
                    | LambdaAbstraction (var, internalTerm) ->
                        substitution internalTerm var sndTerm
                    | Variable _ ->
